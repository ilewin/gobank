...
DELETE FROM entries WHERE account_id = $1 RETURNING id, account_id, amount, created_at
`

func (q *Queries) DeleteEntries(ctx context.Context, accountID int64) ([]Entry, error) {
	rows, err := q.db.QueryContext(ctx, deleteEntries, accountID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
SELECT id, account_id, amount, created_at FROM entries WHERE account_id = $1 ORDER BY id DESC LIMIT $2 OFFSET $3
`
